...
resource "cloudtower_vm" "tf_test" {
  name                = "tf-test-vm-to-be-converted-to-template"
...
resource "cloudtower_vm_template" "tf_test_template_convert_from_vm" {
  name  = "tf-test-template-by-converted-from-vm"
  cloud_init_supported = false
  description = "first tf template"
  create_effect {
    convert_from_vm = cloudtower_vm.tf_test.id
  }
}
